...
namespace cppm::package
{
    ...
    void Package::add_dependency(Config& config) {
        Dependency dep;
        dep.name = name;
        dep.cmake_name = cmake.name;
        dep.version = version;
        dep.components = cmake.components;
        auto table = cpptoml::parse_file(config.path.root + "/cppm.toml");
        dep.insert(table);
        std::fstream file(config.path.root + "/cppm.toml");
        file << table;
        file.close();
    }
    ...
    std::string Package::generate() {
        ...
        return "cmake_minimum_required(VERSION 3.10)\n"
           ...
           //+ "BINARY_DIR ${{CMAKE_BINARY_DIR}}/repo/{0}/build\n"_format(name)
            + "{0}CMAKE_ARGS {1} {2}\n"_format(is_default(cmake.option), "-DCMAKE_INSTALL_PREFIX=$ENV{HOME}/.cppm/local", cmake.option)
