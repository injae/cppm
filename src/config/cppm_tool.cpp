...
namespace cppm::tool
{
    ...
    std::string target_define(Config::ptr config) {
        ...
        auto exclude = [](bool flag) { return flag ? " EXCLUDE" : ""; };
        for(auto& lib : config->libs.list) {
            ...
            gen += "\ncppm_target_define({} {} {}{} {})\n"_format( lib.name, type
                                                                  , optional_define(lib.exclude_flag) 
                                                                  , exclude(lib.exclude) ,gen_sources);
        }

        for(auto& bin : config->bins.list) {
            auto sources = bin.get_sources(config);
            gen += "\ncppm_target_define({} {} {}{}\nSOURCES {}\n)\n"_format(
                   bin.name, "BINARY", optional_define(bin.exclude_flag)
                   , exclude(bin.exclude),  util::accumulate(sources, "\n\t"));
        }
